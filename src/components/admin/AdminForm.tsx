...
function AdminForm() {
  ...
  const onSubmit = async (data) => {
    ...
    const filteredData: AdminData = Object.keys(data)
      .filter(
        (key) =>
          key === "admin_contact" ||
          key === "admin_password" ||
    ...
    <div className="my-[2rem] lg:my-0">
      <form ref={formRef} onSubmit={handleSubmit(onSubmit, onError)}>
        <div className="mb-[3.5rem] relative">
          ...
          <input
            className="w-[35rem] sm:w-[50rem] lg:w-[75rem] h-[5rem] rounded-[1rem] border-2 border-primary-dark px-[1rem] mt-[1rem] shadow-md"
            type="text"
            id="admin_contact"
